...
public class SSDataCenter {
	...
	// el punto especifico se lo da el DC o Ciudad que correspondan desde
	// Sistema
	...
	// PRE: Las coordenadas pertenecen a un DC y no una Ciudad
	public Retorno procesarInformación(Double coordX, Double coordY, int esfuerzoCPUrequeridoEnHoras) {
		...
		if (ori != -1) {
			...
			if (dcOrigen.getCapacidadCPUenHoras() < esfuerzoCPUrequeridoEnHoras) // Si
																					// el
																					// origen
																					// lo
																					// puede
																					...
																					// solo
			{
				...
				// Pide la empresa porque lo tiene que comparar para calcular el
				// costo final;
				DataCenter dcDestino = dDcMP.obtenerDCcapableConMenorDistancia(dcOrigen.getEmpresa(),
			...
			ret.valorString = "No existen las coordenadas";
		return ret;
	}
